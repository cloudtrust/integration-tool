...
logger = logging.getLogger('acceptance-tool.tests.business_tests.test_CT_TC_WS_FED_BROKER_ACCESS_CONTROL_ABAC_KO')
logger.setLevel(logging.DEBUG)


@pytest.mark.usefixtures('settings', 'import_realm', 'import_realm_external')
class Test_CT_TC_WS_FED_BROKER_ACCESS_CONTROL_ABAC_KO():
    """
    Class to test the CT_TC_WS_FED_BROKER_ACCESS_CONTROL_ABAC_KO use case:
    As a resource owner of company A, I need the solution to prevent end users of company B, switching between
    applications of company A in a timeframe smaller than the allowed single sign on time span, after an
    authentication on company B IDP, to access applications they are not entitled to access.
    Company A applications are protected by CloudTrust which acts as a broker.
    """

    def test_CT_TC_WS_FED_BROKER_ACCESS_CONTROL_ABAC_KO_SP_initiated(self, settings):
        """
    ...
    def test_CT_TC_WS_FED_BROKER_ACCESS_CONTROL_ABAC_KO_IDP_initiated(self, settings):
        """
        ...
        # We check that the test works for both types of identity provider
        idp_brokers = [settings["idp"]["saml_broker"], settings["idp"]["wsfed_broker"]]
