...
class Test_test_CT_TC_SAML_IDP_ACCESS_CONTROL_ABAC_OK():
    """
    ...
    I need the solution to grant access to applications whose access I am entitled to have without re-authenticating.
    """

    def test_CT_TC_SAML_IDP_ACCESS_CONTROL_ABAC_OK_SP_initiated(self, settings):
        """
        ...
        if response.status_code == HTTPStatus.UNAUTHORIZED and response.headers['WWW-Authenticate'] == 'Negotiate':
            response = req.kerberos_form_fallback(logger, s, response, header, {**keycloak_cookie, **session_cookie})
